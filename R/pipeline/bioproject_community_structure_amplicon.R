#This Rscript allows to obtain the microbial community structure from Amplicon (16S and ITS) BioProjects
#Using gblm method
input_folder <- "/input_path/"
output_folder <- "/output_path/"
#get the list of bioprojrojects with colmun 1 = bioprojectID, column 2 = assayType, column 3 = isolationSource
csv_file <- read.csv("WMS.csv", header = FALSE, sep = ",")
print(csv_file)
  ...
  tryCatch (
    {
      ...
      features <- subset(features, select=-c(s__))
        ...
        tryCatch(
          {
            ...
            for (i in 1:ntaxa){
              ...
              cvm<-cvrisk(model1,folds=f,mc.cores=24)
              opt_m<-mstop(cvm)
              if(opt_m==0){opt_m=1}
